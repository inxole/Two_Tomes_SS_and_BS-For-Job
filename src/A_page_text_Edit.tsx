...
export const oneTextFreeEdit = async (scene: Scene, updated_text: string, text_size: number, target: number) => {
    ...
    let lineIndex = 0
    if (target % 2 == 1) {
        const book1_front_number = (target - 1) / 2 // 奇数のtargetの場合
        const book2_front_number = 50 + (target - 1) / 2 // 50を基準に計算
